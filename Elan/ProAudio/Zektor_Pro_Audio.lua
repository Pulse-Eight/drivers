...
----For Triggers and other fucntions

--funcName = ("Trigger 01 On")
if string.find(funcName, "Trigger")  then
local trignum = string.sub(funcName,9,10)
--print(trignum)
local statename = string.sub(funcName,12,13)
--print(statename)
local state
	if statename == ("On") then state = string.format("1")
		elseif statename == ("Of") then state = string.format("0")
		elseif statename == ("Zo") then state = string.format("2") end
--print(state)
local sCmd = string.format("^STRG @%d,%d$%c", trignum, state, 0x0D)
SendCommand(sCmd) 
end
end
-------------------	
...
local sTrace = string.format("EDRV_ZonePower: zone %d pwr %d", zone_index, turn_on_off)
	...
	if (turn_on_off == 0) then 						--off
		SendCommand(sOff)
		ELAN_Sleep(100)
		ELAN_Trace(string.format('Sleeping 100ms after OFF Command'))
	else											--on
...
elseif string.find(data, "=SZ")  then
		...
		ELAN_Trace(string.format("INCOMING AZONE: %s", sZone))
		ELAN_Trace(string.format("INCOMING ASRC: %s", source))
		ELAN_RegisterZoneSource(zone_index, source_index)
		ELAN_Trace(string.format(" zidx: %s  , sidx: %s", 	zone_index, source_index))
		ELAN_SetDeviceState('OK')
...
elseif string.find(data, "=P 1$")  then
		ELAN_SetDeviceState('OK')
elseif string.find(data, "=P 0$")  then
		ELAN_SetDeviceState('DISCONNECTED')
elseif string.find(data, "=P 3$")  then
		ELAN_SetDeviceState('OK')
...
elseif string.find(data, "DSZ")  then
		...
		ELAN_Trace(string.format("INCOMING DZONE: %s", dZone))
...
driver = ELAN_GetDriverName()
	if string.find(driver, "IP")  then
		ELAN_Trace('Is IP Driver')
		socket = ELAN_ConnectTCP()

		if (socket) then 
			ELAN_Trace('Socket is Open')
