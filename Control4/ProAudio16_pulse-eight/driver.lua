...
function pxSZ( paramT)
  ...
  if (zone > 16) then
    if (input >= 1 and input <= 16) then
      print( "Digital outputs cannot be switched to analog inputs, output " .. zone .. " muted.")
    end

    sendZoneSource( "DSZ", zone - 16, input, audioF, videoF)
    sendZoneSource( "DMZ", zone - 16, 0, audioF, videoF)
...
function pxDO( paramT)
  ...
  -- xxx
  if (zone > 16) then
    sendZoneSource( "DMZ", zone - 16, 1, audioF, videoF)
  else
...
--  if ((aAudioF + dAudioF) ~= 0) then
    ...
    loczone = zone + 3999 + 16      -- audio output bindings
